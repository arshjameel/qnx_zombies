 ...
 * QNX-specific. bit0=a(top) bit1=b(top-right) bit2=c(bottom-right)
                    ...
                    int win_w, int win_h, f32 r, f32 g, f32 b)
{
    ...
    if (seg & 0x01) push_quad_2d(vl, px, py, px + pw, py + thick, win_w, win_h, r, g, b);                     /* a: top */
    if (seg & 0x02) push_quad_2d(vl, px + pw - thick, py, px + pw, mid_y, win_w, win_h, r, g, b);             /* b: top-right */
    if (seg & 0x04) push_quad_2d(vl, px + pw - thick, mid_y, px + pw, py + ph, win_w, win_h, r, g, b);        /* c: bottom-right */
    if (seg & 0x08) push_quad_2d(vl, px, py + ph - thick, px + pw, py + ph, win_w, win_h, r, g, b);           /* d: bottom */
    if (seg & 0x10) push_quad_2d(vl, px, mid_y, px + thick, py + ph, win_w, win_h, r, g, b);                  /* e: bottom-left */
    if (seg & 0x20) push_quad_2d(vl, px, py, px + thick, mid_y, win_w, win_h, r, g, b);                       /* f: top-left */
    if (seg & 0x40) push_quad_2d(vl, px, mid_y - thick * 0.5f, px + pw, mid_y + thick * 0.5f, win_w, win_h, r, g, b); /* g: middle */
}
                     ...
                     int win_w, int win_h, f32 r, f32 g, f32 b)
{
    ...
    if (value < 0) value = 0;   /* every HUD value (health/ammo/wave/count) is non-negative */
...
void hud_push_crosshair(VertexList *vl, int win_w, int win_h, f32 r, f32 g, f32 b)
{
    ...
    push_quad_2d(vl, cx - half_len, cy - half_thick, cx + half_len, cy + half_thick, win_w, win_h, r, g, b);
    push_quad_2d(vl, cx - half_thick, cy - half_len, cx + half_thick, cy + half_len, win_w, win_h, r, g, b);
}
